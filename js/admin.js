    /**
     * -ONLOAD- Funciones y parametros que hay que cargar cuando se visualiza el panel de administración por completo
     */
    $(function()
    {
        $(".side").click(Clique);
    ...
    /**
     * Permite cerrar la sesión y volver al inicio de la web del CMS
     */
    function Cerrar() {
        $.get("api/?close").done(function (data) {
            toastInfo("Sesión cerrada", function () {
                location.href = ".";
            });
        });
    }
    ...
    function VerificarConexion() {
        $.get("api/?current_user").done(function (data) {
            ...
            if (js.indexOf("Necesitas registrarte como usuario y tener ese permiso") == -1) {
                ...
                toastInfo("Se ha perdido la autentificación con el usuario", function () {
                    location.href = ".";
                });
            }
        });
        ...
        $.get("api/?Allarticle").done(function (data) {
            ...
            $.get("api/?getComments").done(function (data) {
                ...
                $("#mod_comment").on("change", function () {
                    $.post("api/", {
                        set_moderation: $("#mod_comment").val()
                    }).done(function (info) {
                        if (info) {
    ...
    function Tema() {
        $.get("api/?getThemes").done(function (data) {
            var contenedor = "<div class='temas'><h3>Temas</h3><label>Subir un Tema (.ZIP) </label><br /><input id='uploadTema' type='file' />&nbsp;<button class='btn theme_upload' style='margin-bottom:10px'><i class='fa fa-upload' aria-hidden='true' /></button><br /><button class='btn custom_theme' style='margin-bottom:10px'>Personalizar Estilos</button>";
            ...
            $(".theme_upload").click(UploadTheme);
        });
    ...
    function Plugin() {
        ...
        for (var plugin of objeto_pl) {
            ...
            {
                activado = "style='color:red;' title='Desactivado' plugin='"+plugin.name+"'";
            }
            cont += "<div class='acc_plugin'><i class=\"fa fa-check-square-o ablePlugin\" "+activado+" aria-hidden=\"true\"></i><span class='titulo'>" + plugin.name + "</span><i class=\"fa fa-trash unPlugin\" aria-hidden=\"true\" plugin='"+plugin.name+"'></i><p class='parr'><b>Descripción:</b> " + plugin.description + "</p><p class='parr'><b>Autor:</b> " + plugin.author + "</p><p class='parr'><b>Imagen:</b><br /><img src='" + plugin.image + "' style='max-width:150px;max-height: 100px;width:80%;border-radius:5px;border:solid 1px darkgrey' /></p><p class='parr'><b>URL:</b> <a target='_blank' href='" + plugin.url + "'>" + plugin.url + "</a></p></div>";
        }
        ...
        $(".plugin_upload").click(UploadPlugin);
        $(".ablePlugin").click(ActivePlugin);
        $(".unPlugin").click(UninstallPlugin)
    ...
    function btn_reset() {
        if (confirm("Está acción eliminará los usuarios, el tema seleccionado - no el tema en sí - , el titulo y descripción de Firework ¿Está seguro de querer continuar?")) {
            $.get("api?removeConfigFile").done(function (info) {
                if (info) {
                    toastInfo("Se elimino correctamente el fichero de configuración", function () {
                        location.reload();
                    });
                }
            });
